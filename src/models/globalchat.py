...
async def badge_build(bot: commands.Bot, message: discord.Message):
    ...
    try:
        if (
            ...
            in bot.get_guild(1343124570131009579).get_member(message.author.id).roles
        ):
            return "🛠️"
    except:
        return "😀"
...
async def send_one_global(
    ...
    async with aiohttp.ClientSession() as session:
        ...
        user_name = f"[{bag}] {message.author.display_name[:10]} | [{em}] {message.guild.name.replace('discord', 'disc**d')} | ({message.author.id})"
        ...
        if not message.attachments == [] or ref_msg or not message.stickers == []:
            ...
            if ref_msg:
                embed.add_field(
                    name=f"返信: {ref_msg.author.display_name.split(' | ')[0]}",
                    value=ref_msg.content,
                    inline=False,
                )

            try:
                msg = await webhook_object.send(
