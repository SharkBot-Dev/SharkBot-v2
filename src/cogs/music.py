...
class MusicView(discord.ui.LayoutView):
    container = discord.ui.Container(
        ...
        discord.ui.ActionRow(
            discord.ui.Button(emoji="💿", custom_id="music_add+"),
            discord.ui.Button(emoji="⏭️", custom_id="music_skip+"),
            discord.ui.Button(emoji="⏹️", custom_id="music_stop+"),
            discord.ui.Button(emoji="📝", custom_id="music_quote+"),
            discord.ui.Button(emoji="❓", custom_id="music_help+"),
        ),
...
class YTDLSource:
    ...
    def create_ffmpeg_player(cls, url: str, boost: bool = False, volume: int = 70):
        ...
        return discord.FFmpegPCMAudio(
            url, before_options=cls.BASE_FFMPEG, options=options
        )
...
class MusicCog(commands.Cog):
    ...
    async def add_to_queue(self, guild_id, item):
        await self.music_queue_collection.update_one(
            {"guild_id": guild_id}, {"$push": {"queue": item}}, upsert=True
        )
    ...
    async def clear_queue(self, guild_id):
        await self.music_queue_collection.update_one(
            {"guild_id": guild_id}, {"$set": {"queue": []}}, upsert=True
        )
    ...
    async def play_next(self, interaction, guild_id):
        ...
        if not queue_item:
            await self.db["NowPLay"].delete_one({"Guild": interaction.guild.id})
            return
        ...
        audio = YTDLSource.create_ffmpeg_player(
                ...
                if custom_id.startswith("music_skip+"):
                    ...
                    if not q_list:
                        ...
                        desc = "\n".join(
                            [
                                f"{i + 1}. {info['title']}"
                                for i, info in enumerate(q_list)
                            ]
                        )
                        await interaction.response.send_message(
                            embed=make_embed.success_embed(
                                title="現在のキュー", description=desc
                            ),
                            ephemeral=True,
                        )
                    ...
                    class MusicAddModal(discord.ui.Modal):
                        def __init__(self_):
                            super().__init__(
                                title="音楽をキューに追加 (SoundCloudのみ)", timeout=180
                            )
                        ...
                        async def on_submit(self_, interaction: discord.Interaction):
                            assert isinstance(
                                self_.musicurl.component, discord.ui.TextInput
                            )
                            ...
                            if not (
                                host == "soundcloud.com"
                                or (host and host.endswith(".soundcloud.com"))
                            ):
                                return await interaction.response.send_message(
                            ...
                            guild_conf = await self.music_queue_collection.find_one(
                                {"guild_id": interaction.guild.id}
                            )
                            if guild_conf:
                                boost_enabled = (
                                    guild_conf and guild_conf.get("boost") == "true"
                                )
                                vol_setting = guild_conf.get("volume", 70)
                            else:
                                boost_enabled = False
                            ...
                            source_info = await YTDLSource.create_source(
                                self_.musicurl.component.value
                            )
                            ...
                            async def add_to_queue(guild_id, item):
                                await interaction.client.async_db[
                                    "Main"
                                ].MusicQueue.update_one(
                                    {"guild_id": guild_id},
                                    {"$push": {"queue": item}},
                                    upsert=True,
                                )
                            ...
                            if not voice.is_playing():
                                ...
                                voice.play(
                                    ...
                                    after=lambda e: asyncio.run_coroutine_threadsafe(
                                        self.play_next(
                                            interaction, interaction.guild.id
                                        ),
    ...
    music = app_commands.Group(name="music", description="音楽関連のコマンドです。")
    ...
    async def music_play(
        ...
        if ファイル:
            ...
            if not ファイル.filename.endswith((".mp3", ".wav", ".mp4")):
                return await interaction.response.send_message(
                    content="そのファイルは対応していません。\n対応ファイル: `.mp3`, `.wav`, `.mp4`"
                )

            await interaction.response.defer()

            item = {"title": ファイル.filename, "url": ファイル.url, "source": "file"}
            ...
            if not voice.is_playing():
                ...
                audio = YTDLSource.create_ffmpeg_player(
                    item["url"], boost=boost_enabled, volume=vol_setting
                )
                voice.play(
                    audio,
                    after=lambda e: asyncio.run_coroutine_threadsafe(
                        self.play_next(interaction, interaction.guild.id), self.bot.loop
                    ),
                )
        ...
        if url:
            ...
            if host == "soundcloud.com" or (host and host.endswith(".soundcloud.com")):
                ...
                if not voice.is_playing():
                    audio = YTDLSource.create_ffmpeg_player(
                        source_info["url"], boost=boost_enabled, volume=vol_setting
                    )
                    voice.play(
                        audio,
                        after=lambda e: asyncio.run_coroutine_threadsafe(
                            self.play_next(interaction, interaction.guild.id),
                            self.bot.loop,
                        ),
                    )
                ...
                return await interaction.followup.send(view=MusicView())
            elif host == "shugiintv.go.jp" or (
                host and host.endswith(".shugiintv.go.jp")
            ):
                await interaction.response.defer()
    ...
    async def music_queue(self, interaction: discord.Interaction):
        ...
        if not q_list:
            ...
            desc = "\n".join(
                [f"{i + 1}. {info['title']}" for i, info in enumerate(q_list)]
            )
            await interaction.response.send_message(
                embed=make_embed.success_embed(title="現在のキュー", description=desc)
            )

    @music.command(name="boost", description="低音ブーストをします。")
    @app_commands.allowed_contexts(guilds=True, dms=False, private_channels=True)
    @app_commands.checks.cooldown(2, 10, key=lambda i: i.guild_id)
    async def music_boost(self, interaction: discord.Interaction, 有効化するか: bool):
        await self.music_queue_collection.update_one(
            {"guild_id": interaction.guild.id},
        ...
        await interaction.response.send_message(
            embed=make_embed.success_embed(
                title=f"低音ブーストを {'有効化' if 有効化するか else '無効化'} しました。",
                description="SoundCloudとファイルのみ適用されます。",
            )
        )
    ...
    async def music_source(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            ...
            .add_field(
                name="ファイル", value=".mp3などが対応しています。", inline=False
            )
        )
