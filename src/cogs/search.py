...
class MessageSearchPagination(discord.ui.View):
    def __init__(self, interaction: discord.Interaction, query: str, user=None, has_image=False, timeout: float = 60.0):
    ...
    async def fetch_and_edit(self, interaction: discord.Interaction):
        search_params = {
            "content": self.query,
            "limit": self.per_page,
        ...
        if self.user:
            search_params["author_id"] = self.user.id
        if self.has_image:
            search_params["has"] = "image"

        try:
            messages = await self.interaction.client.raw(self.interaction.client).search_messages(self.interaction.guild.id, **search_params)
            
            if not messages and self.current_page > 0:
                self.current_page -= 1
                return await interaction.response.send_message("これ以上の結果はありません。", ephemeral=True)

            embed = discord.Embed(
                title=f"「{self.query}」の検索結果",
                color=discord.Color.blue(),
                description=f"ページ {self.current_page + 1} の表示中"
            )

            for msg in messages:
                content = msg.content[:100] or "[内容なし]"
                embed.add_field(
                    name=f"{msg.author.display_name} ({msg.created_at.strftime('%Y/%m/%d')})",
                    value=f"{content}\n[ジャンプ]({msg.jump_url})",
                    inline=False
                )

            self.previous_button.disabled = (self.current_page == 0)
            self.next_button.disabled = (len(messages) < self.per_page)
...
""",
    ...
    @search.command(name="messages", description="サーバー内のメッセージを検索します。")
    @app_commands.allowed_contexts(guilds=True, dms=False, private_channels=True)
    @app_commands.checks.cooldown(2, 10, key=lambda i: i.guild_id)
    async def search_messages(self, interaction: discord.Interaction, 内容: str, ユーザー: discord.User = None, 画像があるか: bool = None):
        if interaction.is_user_integration() and not interaction.is_guild_integration():
            return await interaction.response.send_message(
                ephemeral=True,
                embed=make_embed.error_embed(
        ...
        await interaction.response.send_message(ephemeral=True, embed=make_embed.loading_embed(title="検索中です..", description="しばらくお待ちください。"))

        view = MessageSearchPagination(interaction, 内容, ユーザー, 画像があるか)
        search_params = {
            "content": 内容,
            "limit": 5
        }
        ...
        if ユーザー:
            search_params["author_id"] = ユーザー.id
        if 画像があるか:
            search_params["has"] = "image"

        messages = await self.bot.raw(self.bot).search_messages(interaction.guild.id, **search_params)

        if not messages:
            return await interaction.edit_original_response(embed=make_embed.error_embed(title="メッセージが見つかりませんでした。", description="検索ワードを変えて再度お試しください。"))

        embed = discord.Embed(title=f"「{内容}」の検索結果", color=discord.Color.blue())
        for msg in messages:
            embed.add_field(name=f"{msg.author.display_name}", value=f"{msg.content[:100]}\n[ジャンプ]({msg.jump_url})", inline=False)
            
        view.next_button.disabled = (len(messages) < view.per_page)
