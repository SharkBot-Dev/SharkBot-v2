...
class AutoGGCog(commands.Cog):
    ...
    async def on_presence_update(self, before: discord.Member, after: discord.Member):
        ...
        old_game = discord.utils.find(
            lambda a: a.type == discord.ActivityType.playing, before.activities
        )

        is_now_playing = any(
            a.type == discord.ActivityType.playing for a in after.activities
        )
                ...
                async with aiohttp.ClientSession() as session:
                    try:
                        ...
                        await webhook.send(
                            ...
                            embed=discord.Embed(
                                title="プレイしていたゲーム", color=discord.Color.blue()
                            ).add_field(name="ゲーム名", value=game_name),
                        )
                    except (discord.NotFound, discord.Forbidden):
                        await db.update_one(
                            {"Guild": after.guild.id}, {"$set": {"Enabled": False}}
                        )

    autogg = app_commands.Group(
        name="autogg",
        description="ゲームをやめた際にggと送信します。",
        allowed_installs=app_commands.AppInstallationType(guild=True, user=False),
    )

    @autogg.command(
        name="setup",
        description="AutoGGのON/OFFを切り替え、現在のチャンネルに設定します。",
    )
    ...
    async def autogg_channel(self, interaction: discord.Interaction):
        ...
        await db.update_one(
            {"Guild": interaction.guild.id}, {"$set": update_data}, upsert=True
        )
