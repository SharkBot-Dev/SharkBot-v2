...
YOUTUBE_ID_REGEX = re.compile(
    r".*(?:youtu\.be\/|v\/|vi\/|e\/|embed\/|watch\?v=|watch\?.+&v=)([^#&?]{11}).*"
)
...
class YoutubeCog(commands.Cog):
    ...
    async def youtube_add_alert(
        ...
        youtubeチャンネルid: str,
        メンション: discord.Role = None,
    ):
        if not CHANNEL_REGEX.match(youtubeチャンネルid):
            return await interaction.response.send_message(
                embed=make_embed.error_embed(
                    title="不正なYoutubeチャンネルidです。",
                    description="正しいYoutubeチャンネルIDを入れてください。",
                )
            )

        db = interaction.client.async_db["MainTwo"].YoutubeAlert
        if await db.find_one(
            {"channel_id": youtubeチャンネルid, "guild_id": interaction.guild.id}
        ):
            return await interaction.response.send_message(
                embed=make_embed.error_embed(title="すでに登録されています。")
            )
        ...
        if count >= 3:
            return await interaction.followup.send(
                embed=make_embed.error_embed(
                    title="三つまでしか登録できません。",
                    description="不要なYoutube通知を削除してください。",
                )
            )
        ...
        await interaction.followup.send(
            embed=make_embed.success_embed(
                title="登録しました。", description=f"ステータスコード: {res}"
            )
        )

    @youtube.command(name="remove", description="Youtube通知から削除します。")
    @app_commands.allowed_contexts(guilds=True, dms=False, private_channels=True)
    @app_commands.checks.cooldown(2, 10, key=lambda i: i.guild_id)
    @app_commands.checks.has_permissions(manage_channels=True)
    async def youtube_remove_alert(
        self, interaction: discord.Interaction, youtubeチャンネルid: str
    ):
        db = interaction.client.async_db["MainTwo"].YoutubeAlert
        await interaction.response.defer()
    ...
    async def youtube_list_alert(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            embed=make_embed.success_embed(title="登録チャンネル一覧", description=text)
        )

    @youtube.command(name="thumbnail", description="Youtube動画のサムネを取得します。")
    @app_commands.allowed_contexts(guilds=True, dms=False, private_channels=True)
    @app_commands.checks.cooldown(2, 10, key=lambda i: i.guild_id)
    async def youtube_thumbnail(self, interaction: discord.Interaction, 動画のurl: str):
        ...
        embed = make_embed.success_embed(
            title="サムネイルを取得しました。", description=f"動画ID: `{v_id}`"
