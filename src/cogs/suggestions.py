...
class SuggestionsSendModal(discord.ui.Modal, title="提案を送信する。"):
    title_ = discord.ui.Label(
        text="タイトル",
        description="タイトルを入力",
        component=discord.ui.TextInput(style=discord.TextStyle.short, required=True),
    )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if not dbfind:
            return await interaction.followup.send(
                embed=make_embed.error_embed(
                    title="提案チャンネルが見つかりません。",
                    description="管理者に問い合わせください。",
                )
            )
        ...
        msg = await channel.send(
            embed=discord.Embed(
                ...
                description=self.text.component.value,
...
class SuggestionsCog(commands.Cog):
    ...
    async def on_reaction_add_suggestions(
        ...
        try:
            dbfind = await db.find_one(
                {"Guild": guild.id, "Channel": payload.channel_id}, {"_id": False}
            )
        except Exception:
            return
        ...
        match = re.search(
            r"可決に必要な賛成数:\s*(\d+).*否決に必要な反対数:\s*(\d+)", footer
        )
        if not match:
            return
    ...
    async def suggestions_send(self, interaction: discord.Interaction):
        db = interaction.client.async_db["MainTwo"].Suggestions
        try:
            dbfind = await db.find_one({"Guild": interaction.guild.id}, {"_id": False})
        except Exception:
    ...
    @suggestions.command(
        name="setting", description="提案関連のチャンネルをセットアップします。"
    )
            ...
            await db.update_one(
                {"Guild": interaction.guild.id},
                {
                    "$set": {
                        ...
                        "MaxCount": 可決されるのに必要な賛成数,
                        "MinCount": 否決されるのに必要な賛成数,
                    }
                },
                upsert=True,
            )
