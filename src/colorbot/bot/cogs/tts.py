...
class DictGroup(app_commands.Group):
    ...
    async def dict_add(
        ...
        ttsdict = interaction.client.async_db.TTSWord
...
class TTSCog(commands.Cog):
    ...
    async def _tts_start(self, interaction: discord.Interaction):
        ...
        try:
            ...
            return
        ttscheck = self.bot.async_db.TTSCheckBeta
...
""",
    ...
    async def tts_guilds(self):
        db = self.bot.async_db.TTSCheckBeta
    ...
    async def tts_autojoin(
        ...
        await interaction.response.defer()
        ttscheck = self.bot.async_db.TTSAutoJoin
        if 有効化するか:
            await ttscheck.update_one(
    ...
    async def replace_word(self, word: str, guild: discord.Guild):
        w = word
        ttsdict = self.bot.async_db.TTSWord
