...
class HelpView(discord.ui.View):
    ...
    def add_cmd(self, name: str, description: str, sub: list | None = None):
        self.cmds.append(
            {"name": name, "description": description or "説明なし", "sub": sub or []}
        )
...
class CategorySelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            embed=await self.view_help.build_embed(), view=self.view_help
        )
...
class CommandSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        cmd = next(
            c
            for c in self.view_help.current_category["sub"]
            if c["name"] == self.values[0]
        )
...
class PageButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        if self.view_help.current_category is None:
            ...
            self.view_help.category_page = max(
                0, min(self.view_help.category_page + self.direction, max_p)
            )
        else:
            chunks = chunk_list(self.view_help.current_category["sub"], 10)
            max_p = len(chunks) - 1
            self.view_help.command_page = max(
                0, min(self.view_help.command_page + self.direction, max_p)
            )
        ...
        if val == "close":
            ...
            await interaction.followup.send(
                embed=discord.Embed(
                    ...
                    description="以下のボタンから招待できます。",
                ),
                view=discord.ui.View().add_item(
                    discord.ui.Button(
                        label="招待リンク",
                        url=f"https://discord.com/oauth2/authorize?client_id={interaction.client.user.id}&permissions=8&integration_type=0&scope=bot+applications.commands",
                    )
                ),
                ephemeral=True,
            )
