...
export default async function UserInstallMiq({
    ...
    async function createNewApp(formData: FormData) {
        ...
        const gemkey = formData.get("gemkey")?.toString();
        const prompt = formData.get("prompt")?.toString();
        if (!cid || !csc || !pubk || !gemkey || !prompt) return;
          ...
          body: JSON.stringify([
            {
              name: "会話",
              type: 1,
              description: "キャラクターと会話します。",
              options: [
                {
                    "name": "発言",
                    "description": "AIに発言する、発言を入力してください。",
                    "type": 3,
                    "required": true
                }
              ]
        ...
        redirect(`/dashboard/settings/${guildid}/appmaker/roleplay/settings/${cid}`);
    }
    ...
    async function deleteApp(formData: FormData) {
        ...
        redirect(`/dashboard/settings/${guildid}/appmaker/roleplay/`);
    }
  ...
  return (
    <div className="p-4 space-y-6">
      ...
      <div>
        <h2 className="text-xl font-semibold mb-2">登録済みロールプレイアプリ</h2>
        ...
        {apps.length === 0 ? (
          ...
          <ul className="space-y-2">
            {apps.map((app) => (
              ...
              >
                <a href={`/dashboard/settings/${guildid}/appmaker/roleplay/settings/${app.AppID}`}><div>
