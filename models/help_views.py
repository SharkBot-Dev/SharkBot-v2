    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if val == "close":
            ...
            await interaction.followup.send(embed=discord.Embed(title="Botを招待する", color=discord.Color.green(), description="以下のボタンから招待できます。"), view=discord.ui.View()
                                            .add_item(discord.ui.Button(label="招待リンク", url="https://discord.com/oauth2/authorize?client_id=1322100616369147924&permissions=8&integration_type=0&scope=bot+applications.commands")),ephemeral=True)