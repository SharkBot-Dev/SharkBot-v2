...
class PremiumCog(commands.Cog):
    ...
    premium = app_commands.Group(name="premium", description="寄付者プランを管理します。",
    ...
    async def premium_free(
        ...
        find_premium = await self.col.find_one({
            "user_id": interaction.user.id
        })
        if not find_premium is None:

            if find_premium.get('is_premium'):
                await interaction.followup.send(ephemeral=True, embed=make_embed.error_embed(title="すでに寄付者です。", description="無料トライアルは寄付者以外が使用できます。"))
                return

            if find_premium.get('is_used_free'):
                await interaction.followup.send(ephemeral=True, embed=make_embed.error_embed(title="すでに参加したことがあるようです", description="無料トライアルは終了しました。"))
                return
        ...
        await interaction.followup.send(ephemeral=True, embed=make_embed.success_embed(title="無料トライアルに参加しました。", description="三日後にトライアルは終了します。"))
        return
