...
class GroupCog(commands.Cog):
    ...
    async def group_create(self, interaction: discord.Interaction, グループ名: str, 説明: str):
    ...
    @group.command(name="setrule", description="グループのルールを設定します（オーナー専用）。")
    @app_commands.allowed_contexts(guilds=True, dms=False, private_channels=True)
    @app_commands.checks.cooldown(2, 10, key=lambda i: i.guild_id)
    async def group_setrule(self, interaction: discord.Interaction, グループ名: str, ルール: str):
        db = self.bot.async_db["Main"].Group
        dbfing = await db.find_one({"Name": グループ名})
    ...
    async def group_leave(self, interaction: discord.Interaction, グループ名: str):
        ...
        await db.update_one(
            {"Name": グループ名},
            {'$pull': {'Member': interaction.user.id}}
        )
    ...
    async def group_list(self, interaction: discord.Interaction):
        ...
        description = "\n".join([f"・**{g['Name']}** - {g.get('Text', '説明なし')}" for g in groups])
        return await interaction.response.send_message(
            embed=make_embed.success_embed(
    ...
    @group.command(name="members", description="参加しているグループのメンバーを表示します。")
    @app_commands.allowed_contexts(guilds=True, dms=False, private_channels=True)
    @app_commands.checks.cooldown(2, 10, key=lambda i: i.guild_id)
