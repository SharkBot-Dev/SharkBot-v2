...
class UpCog(commands.Cog):
    ...
    async def add_money(self, message: discord.Message):
        ...
        try:
            ...
            return await message.channel.send(embed=discord.Embed(title="追加に失敗しました。", description=f"以下を管理者権限を持っている人に\n認証してもらってください。\nhttps://unbelievaboat.com/applications/authorize?app_id=1326818885663592015", color=discord.Color.yellow()))
        ...
        if message.author.id == 903541413298450462:
            try:
                if "サーバーは上部に表示されます。" in message.embeds[0].description:
                    ...
                    await self.bot.alert_add("dicoall", message.channel.id, ment, "DicoallをUpしてね！", "</up:935190259111706754> でアップ。", 3600)
        ...
        if message.author.id == 1300797373374529557:
            try:
                if "表示順を上げました" in message.embeds[0].description:
                    ...
                    await self.bot.alert_add("distopia", message.channel.id, ment, "DisTopiaをBumpしてね！", "</bump:1309070135360749620> でBump。", 7200)
            except:
                return
        ...
        if message.author.id == 302050872383242240:
            try:
                if "表示順をアップ" in message.embeds[0].description:
                    ...
                    await message.reply(embed=discord.Embed(title="Bumpを検知しました。", description=f"二時間後に通知します。\n以下のロールに通知します。\n{ment}", color=discord.Color.green()))
                    # await asyncio.sleep(7200)
                    await self.bot.alert_add("disboard", message.channel.id, ment, "DisboardをBumpしてね！", "</bump:947088344167366698> でBump。", 7200)
        ...
        if message.author.id == 850493201064132659:
            try:
                if "サーバーの表示順位を" in message.embeds[0].description:
                    ...
                    await self.add_money(message)
                    # await asyncio.sleep(3600)
                    await self.bot.alert_add("discafe", message.channel.id, ment, "DisCafeをUpしてね！", "</up:980136954169536525> でUp。", 3600)
            except:
                return
        ...
        if after.author.id == 761562078095867916:
            try:
                if "をアップしたよ!" in after.embeds[0].fields[0].name:
                    ...
                    # await asyncio.sleep(7200)
                    await self.bot.alert_add("dissoku", after.channel.id, ment, "ディス速をUpしてね！", "</up:1363739182672904354> でアップ。", 7200)
    ...
    async def up_dicoall(self, interaction: discord.Interaction, onか: bool):
        ...
        if onか:
            ...
            await interaction.response.send_message(embed=discord.Embed(title="Dicoallの通知をONにしました。", color=discord.Color.green()))
        else:
            await db.delete_one(
                {"Channel": interaction.channel.id}
            )
            await interaction.response.send_message(embed=discord.Embed(title="Dicoallの通知をOFFにしました。", color=discord.Color.red()))
    ...
    async def up_distopia(self, interaction: discord.Interaction, onか: bool):
        ...
        if onか:
            ...
            await interaction.response.send_message(embed=discord.Embed(title="Distopiaの通知をONにしました。", color=discord.Color.green()))
        else:
            await db.delete_one(
                {"Channel": interaction.channel.id}
            )
            await interaction.response.send_message(embed=discord.Embed(title="Distopiaの通知をOFFにしました。", color=discord.Color.red()))
    ...
    async def sabachannel_vote(self, interaction: discord.Interaction, onか: bool):
        ...
        if onか:
            ...
            await interaction.response.send_message(embed=discord.Embed(title="鯖チャンネルの通知をONにしました。", color=discord.Color.green()))
        else:
            await db.delete_one(
                {"Channel": interaction.channel.id}
            )
    ...
    async def dissoku_up(self, interaction: discord.Interaction, onか: bool):
        ...
        if onか:
            ...
            await interaction.response.send_message(embed=discord.Embed(title="ディス速の通知をONにしました。", color=discord.Color.green()))
        else:
            await db.delete_one(
                {"Channel": interaction.channel.id}
            )
            await interaction.response.send_message(embed=discord.Embed(title="ディス速の通知をOFFにしました。", color=discord.Color.red()))
    ...
    async def disboard_bump(self, interaction: discord.Interaction, onか: bool):
        ...
        if onか:
            ...
            await interaction.response.send_message(embed=discord.Embed(title="Disboardの通知をONにしました。", color=discord.Color.green()))
        else:
            await db.delete_one(
                {"Channel": interaction.channel.id}
            )
            await interaction.response.send_message(embed=discord.Embed(title="Disboardの通知をOFFにしました。", color=discord.Color.red()))
    ...
    async def dcafe_up(self, interaction: discord.Interaction, onか: bool):
        ...
        if onか:
            ...
            await interaction.response.send_message(embed=discord.Embed(title="DCafeの通知をONにしました。", color=discord.Color.green()))
        else:
            await db.delete_one(
                {"Channel": interaction.channel.id}
            )
            await interaction.response.send_message(embed=discord.Embed(title="DCafeの通知をOFFにしました。", color=discord.Color.red()))
    ...
    async def mention_up(self, interaction: discord.Interaction, ロール: discord.Role = None):
        ...
        if not ロール:
            ...
            return await interaction.response.send_message(embed=discord.Embed(title="Up・Bump通知時にロールを\n通知しないようにしました。", color=discord.Color.green()))
        await db.replace_one(
            {"Channel": interaction.channel.id},
