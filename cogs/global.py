...
class GlobalCog(commands.Cog):
    ...
    globalchat = app_commands.Group(name="global", description="グローバルチャット系のコマンドです。")
    ...
    async def global_join(self, interaction: discord.Interaction, 部屋名: str = None):
        ...
        if not 部屋名:
            if interaction.guild.member_count < 20:
                return await interaction.followup.send(embed=discord.Embed(title="20人未満のサーバーは参加できません。", color=discord.Color.red()))
            check_room = await self.globalchat_room_check(interaction)
            if check_room:
                await self.globalchat_room_leave(interaction)
                return await interaction.followup.send(embed=discord.Embed(title="グローバルチャットから脱退しました。", color=discord.Color.green()))
            ...
            if check:
                ...
                await interaction.followup.send(embed=discord.Embed(title="グローバルチャットに参加しました。", description="グローバルチャットのルール\n・荒らしをしない\n・宣伝をしない\n・r18やグロ関連のものを貼らない\n・その他運営の禁止したものを貼らない\n以上です。守れない場合は、処罰することもあります。\nご了承ください。", color=discord.Color.green()))
            ...
            if check:
                ...
                await interaction.followup.send(embed=discord.Embed(title="グローバルチャットに参加しました。", color=discord.Color.green()))
    ...
    async def global_leave(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(embed=discord.Embed(title="グローバルチャットから脱退しました。", color=discord.Color.green()))
    ...
    async def global_emoji(self, interaction: discord.Interaction, 絵文字: str):
        ...
        await interaction.followup.send(embed=discord.Embed(title="絵文字を変更しました。", color=discord.Color.green()).add_field(name="絵文字", value=絵文字))
    ...
    async def global_server(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(embed=discord.Embed(title="サーバー掲示板", description="以下のurlからアクセスできます。\nhttps://www.sharkbot.xyz/server", color=discord.Color.blue()))
    ...
    async def global_register(self, interaction: discord.Interaction, 説明: str):
        ...
        if interaction.guild.icon == None:
            return await interaction.reply("サーバー掲示板に乗せるにはアイコンを設定する必要があります。")
        db = self.bot.async_db["Main"].Register
        inv = await interaction.channel.create_invite()
        await db.replace_one(
            {"Guild": interaction.guild.id}, 
            {"Guild": interaction.guild.id, "Name": interaction.guild.name, "Description": 説明, "Invite": inv.url, "Icon": interaction.guild.icon.url}, 
            upsert=True
        )
        await interaction.response.send_message(embed=discord.Embed(title="サーバーを掲載しました。", color=discord.Color.green()))
    ...
    async def get_reg(self, interaction: discord.Interaction):
        ...
        if dbfind is None:
            return "https://discord.com", None
        return dbfind.get("Invite", "https://discord.com"), dbfind.get("Description", "説明なし")
    ...
    async def global_up(self, interaction: discord.Interaction):
        ...
        if interaction.guild.icon == None:
            return await interaction.response.send_message("Upをするにはアイコンを設定する必要があります。")
        
        current_time = time.time()
        last_message_time = user_last_message_timegc.get(interaction.guild.id, 0)
        if current_time - last_message_time < 7200:
            return await interaction.response.send_message(ephemeral=True, embed=discord.Embed(title="まだUpできません。", color=discord.Color.red(), description="2時間待ってください。"))
        ...
        if inv == "https://discord.com":
            return await interaction.response.send_message(embed=discord.Embed(title="まだ登録されていません。", color=discord.Color.red(), description="/global registerで登録してください。"))
        await db.replace_one(
            {"Guild": interaction.guild.id}, 
            {"Guild": interaction.guild.id, "Name": interaction.guild.name, "Description": desc, "Invite": inv, "Icon": interaction.guild.icon.url, "Up": str(time.time())}, 
            upsert=True
        )
        await interaction.response.send_message(embed=discord.Embed(title="サーバーをUpしました！", description="2時間後に再度Upできます。", color=discord.Color.green()))
    ...
    async def global_private(self, interaction: discord.Interaction):
        ...
        class PrivateGlobalCreate(discord.ui.Modal, title="プライベートグローバルチャットを作成する"):
            ...
            async def on_submit(self, interaction: discord.Interaction):
                ...
                if dbfind is None:
                    web = await interaction.channel.create_webhook(name="SharkBot-PrivateGlobal")
                    await db.replace_one(
                        {"Guild": interaction.guild.id, "Name": self.name.value}, 
                        {"Guild": interaction.guild.id, "Name": self.name.value, "Password": self.password.value, "Owner": interaction.user.id, "Channel": interaction.channel.id, "Webhook": web.url}, 
                        upsert=True
                    )
                    await interaction.followup.send(embed=discord.Embed(title=f"プライベートグローバルチャットを作成しました。", color=discord.Color.green()))
                else:
                    await interaction.followup.send(embed=discord.Embed(title=f"その名前 `{self.name.value}` は既に使われています！", description="別の名前を使用してください。", color=discord.Color.red()))
    ...
    async def global_private_join(self, interaction: discord.Interaction):
        ...
        class PrivateGlobalJoin(discord.ui.Modal, title="プライベートグローバルチャットに参加する"):
            name = discord.ui.TextInput(
                label='名前を入力',
                required=True,
                style=discord.TextStyle.short,
            ...
            async def on_submit(self, interaction: discord.Interaction):
                await interaction.response.defer(thinking=True)
                db = interaction.client.async_db["Main"].PrivateGlobal
                dbfind = await db.find_one({"Name": self.name.value, "Password": self.password.value}, {"_id": False})
                if not dbfind is None:
                    web = await interaction.channel.create_webhook(name="SharkBot-PrivateGlobal")
                    await db.replace_one(
                        {"Guild": interaction.guild.id, "Name": self.name.value}, 
                        {"Guild": interaction.guild.id, "Name": self.name.value, "Password": self.password.value, "Owner": dbfind.get("Owner"), "Channel": interaction.channel.id, "Webhook": web.url}, 
                        upsert=True
                    )
                    await interaction.followup.send(embed=discord.Embed(title=f"プライベートグローバルチャットに参加しました。", color=discord.Color.green()))
                else:
                    await interaction.followup.send(embed=discord.Embed(title=f"その名前 `{self.name.value}` のグローバルチャットはありません！", description="またはパスワードが違います。", color=discord.Color.red()))
    ...
    async def global_private_leave(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(embed=discord.Embed(title="グローバルチャットから脱退しました。", color=discord.Color.red()))
    ...
    async def global_private_leave(self, interaction: discord.Interaction):
        ...
        db = self.bot.async_db["Main"].PrivateGlobal
        dbfind = await db.find_one({"Channel": interaction.channel.id, "Owner": interaction.user.id}, {"_id": False})
        if dbfind is None:
            return await interaction.response.send_message(embed=discord.Embed(title="このチャンネルはプライベートグローバルチャットではありません。", description="または、オーナーではありません。", color=discord.Color.red()), ephemeral=True)
        else:
            return await interaction.response.send_message(embed=discord.Embed(title="このグローバルチャットのパスワード", description=f"パスワード: `{dbfind.get("Password", "エラー")}`", color=discord.Color.blue()), ephemeral=True)
    ...
    async def sgc_make_json(self, message: discord.Message):
            ...
            if message.reference:
                ...
                reference_mid = 0
                if reference_msg.embeds and self.bot.user.id == reference_msg.application_id:  #返信の元のメッセージが、埋め込みメッセージかつ、このBOTが送信したメッセージのとき→グローバルチャットの他のサーバーからのメッセージと判断
                ...
                elif reference_msg.author != reference_msg.application_id: #返信の元のメッセージが、このBOTが送信したメッセージでは無い時→同じチャンネルのメッセージと判断
        ...
        async with aiohttp.ClientSession() as session:
            async for channel in channels:
                ...
                if target_channel:
                    ...
                    bag = await self.badge_build(message)
                    if message.author.avatar:
                        embed.set_author(name=f"[{bag}] {message.author.name}/{message.author.id} [{message.author.primary_guild.tag if message.author.primary_guild.tag else "なし"}]", icon_url=message.author.avatar.url)
                    else:
                        embed.set_author(name=f"[{bag}] {message.author.name}/{message.author.id} [{message.author.primary_guild.tag if message.author.primary_guild.tag else "なし"}]", icon_url=message.author.default_avatar.url)
                    if not message.attachments == []:
                        for kaku in [".png", ".jpg", ".jpeg", ".gif", ".webm"]:
                            if kaku in message.attachments[0].filename:
                                embed.set_image(url=message.attachments[0].url)
                                break
                        embed.add_field(name="添付ファイル", value=message.attachments[0].url, inline=False)
                    if message.reference:
                        embed.add_field(name=f"返信 ({rmsg.author.name}#{rmsg.author.discriminator})", inline=False, value=f"{rmsg.content}")
    ...
    async def super_join_global_chat(self, interaction: discord.Interaction):
        ...
        await db.replace_one(
            {"Guild": interaction.guild.id}, 
            {"Guild": interaction.guild.id, "Channel": interaction.channel.id, "GuildName": interaction.guild.name, "Webhook": wh.url},
            upsert=True
        )
        ...
        async with aiohttp.ClientSession() as session:
            async for ch in db.find():
                ...
                if target_channel:
                    ...
                    bag = await self.badge_build(message)
                    if dic["userAvatar"]:
                        embed.set_author(name=f"[{bag}] {dic['userName']}/{dic['userId']} [{dic.get("x-userTag", "不明") if dic.get("x-userTag", None) != None else "なし"}]", icon_url="https://media.discordapp.net/avatars/{}/{}.png?size=1024".format(dic["userId"], dic["userAvatar"]))
                    else:
                        embed.set_author(name=f"[{bag}] {dic['userName']}/{dic['userId']} [{dic.get("x-userTag", "不明") if dic.get("x-userTag", None) != None else "なし"}]", icon_url=message.author.default_avatar.url)
                    ...
                    if message.reference:
                        rmsg = await message.channel.fetch_message(message.reference.message_id)
                        embed.add_field(name=f"返信 ({rmsg.author.name} - {rmsg.author.id})", inline=False, value=f"{rmsg.content}")
                    else:
                        try:
                            ...
                            async for past_message in message.channel.history(limit=1000): #JSONチャンネルの過去ログ1000件をループ
                                ...
                                if str(past_dic["messageId"]) == str(reference_mid): #過去ログのメッセージIDが返信元メッセージIDと一致したとき
                                    ...
                                    embed.add_field(name=f"返信 ({reference_message_author})", inline=False, value=f"{reference_message_content}")
                                    break
                        except:
                            pass
    ...
    async def global_sgc(self, interaction: discord.Interaction):
        ...
        if check:
            ...
            return await interaction.followup.send(embed=discord.Embed(title="スーパーグローバルチャットから脱退しました。", color=discord.Color.green()))
        else:
            await self.super_join_global_chat(interaction)
            await interaction.followup.send(embed=discord.Embed(title="スーパーグローバルチャットに参加しました。", color=discord.Color.green()))
    ...
    async def global_sgc_info(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(embed=discord.Embed(title="SGCのBot情報", color=discord.Color.green(), description=res))
    ...
    async def global_shiritori(self, interaction: discord.Interaction):
        ...
        await interaction.followup.send(embed=discord.Embed(title="グローバルしりとりに参加しました。", description="脱退は、/global leaveを使ってください。", color=discord.Color.green()))
    ...
    async def globalads_join(self, interaction: discord.Interaction):
        ...
        await db.replace_one(
            {"Guild": interaction.guild.id}, 
            {"Guild": interaction.guild.id, "Channel": interaction.channel.id, "GuildName": interaction.guild.name, "Webhook": web.url}, 
            upsert=True
        )
        return True
    ...
    async def send_one_ads_message(self, webhook: str, interaction: discord.Interaction, text: str):
        async with aiohttp.ClientSession() as session:
            webhook_ = Webhook.from_url(webhook, session=session)
            embed = discord.Embed(description=text, color=discord.Color.blue())
            em = await self.get_guild_emoji(interaction.guild)
            embed.set_footer(text=f"[{em}] {interaction.guild.name}/{interaction.guild.id}")

            bag = await self.badge_build(interaction)

            if interaction.user.avatar:
                embed.set_author(name=f"[{bag}] {interaction.user.name}/{interaction.user.id}", icon_url=interaction.user.avatar.url)
            else:
                embed.set_author(name=f"[{bag}] {interaction.user.name}/{interaction.user.id}", icon_url=interaction.user.default_avatar.url)
            try:
                await webhook_.send(embed=embed, avatar_url=self.bot.user.avatar.url, username="SharkBot-Global")
            except:
                return
    ...
    async def global_ads(self, interaction: discord.Interaction):
        ...
        await interaction.response.defer()
        if interaction.guild.member_count < 20:
            return await interaction.response.send_message(embed=discord.Embed(title="20人未満のサーバーは参加できません。", color=discord.Color.red()))
        check = await self.globalads_check(interaction)
        if check:
            await self.globalads_leave(interaction)
            return await interaction.followup.send(embed=discord.Embed(title="グローバル宣伝から脱退しました。", color=discord.Color.green()))
        else:
            await self.globalads_join(interaction)
            await interaction.followup.send(embed=discord.Embed(title="グローバル宣伝に参加しました。", description="グローバル宣伝のルール\n・荒らし系を貼らない\n・r18やグロ関連のものを貼らない\n・sh0p系を貼らない\n・その他運営の禁止したものを貼らない\n以上です。守れない場合は、処罰することもあります。\nご了承ください。", color=discord.Color.green()))
    ...
    async def global_dsgc(self, interaction: discord.Interaction):
        ...
        if check:
            ...
            return await interaction.followup.send(embed=discord.Embed(title="デバッグ版スーパーグローバルチャットから脱退しました。", color=discord.Color.green()))
        else:
            await self.debug_super_join_global_chat(interaction)
            await interaction.followup.send(embed=discord.Embed(title="デバッグ版スーパーグローバルチャットに参加しました。", color=discord.Color.green()))
    ...
    async def send_super_global_chat_room_debug(self, message: discord.Message, ref_msg: discord.Message = None):
        db = self.bot.async_db["Main"].AlpheSuperGlobalChatDebug
        channels = db.find()
    ...
    async def on_message_super_global_debug(self, message: discord.Message):
        ...
        await self.bot.get_channel(707158343952629780).send(content=js, allowed_mentions=discord.AllowedMentions.none())
