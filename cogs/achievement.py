...
class AchievementCog(commands.Cog):
    ...
    async def get_is_achi(self, member: discord.Member, name: str):
        ...
        try:
            dbfind = await db.find_one({"User": member.id, "Guild": member.guild.id, "Name": name}, {"_id": False})
        except:
            return False
        if dbfind is None:
            return False
    ...
    async def send_channel(self, message: discord.Message, achi):
        ...
        try:
            await notify_channel.send(f"{message.author.mention} が実績「{achi.get('Name')}」を達成しました。")
        except Exception:
            pass
    ...
    async def on_message_say(self, message: discord.Message):
        ...
        user_data = await db.find_one({"Guild": message.guild.id, "User": message.author.id})
        ...
        if user_data:
            ...
            new_count = 1
            await db.insert_one(
                {"Guild": message.guild.id, "User": message.author.id, "count": new_count}
            )
    ...
    achievement = app_commands.Group(name="achievement", description="実績関連のコマンドです。")
    ...
    async def achievement_create(self, interaction: discord.Interaction, 実績名: str, 値: int, をする: app_commands.Choice[str]):
        ...
        await db.replace_one(
            ...
            {"Guild": interaction.guild.id, "Name": 実績名, "Value": 値, "If": をする.value},
            upsert=True,
        )
        await interaction.response.send_message(content=f"{値}{をする.name}と、「{実績名}」が入手できるようにしました。", ephemeral=True)
    ...
    async def achievement_delete(self, interaction: discord.Interaction, 実績名: str):
        ...
        if result.deleted_count == 0:
            await interaction.response.send_message(content=f"何も削除されませんでした。", ephemeral=True)
        await interaction.response.send_message(content=f"{実績名}を削除しました。", ephemeral=True)

    @achievement.command(name="channel", description="実績達成通知を送るチャンネルを設定します。")
    @app_commands.checks.has_permissions(administrator=True)
    @app_commands.allowed_contexts(guilds=True, dms=False, private_channels=True)
    async def achievement_channel(self, interaction: discord.Interaction, チャンネル: discord.TextChannel = None):
        ...
        if チャンネル:
            ...
            await interaction.response.send_message(
                f"実績達成通知チャンネルを {チャンネル.mention} に設定しました。", ephemeral=True
            )
        else:
    ...
    async def achievement_show(self, interaction: discord.Interaction, メンバー: discord.User = None):
        ...
        async for b in db.find({"Guild": interaction.guild.id}):
            ...
            if achi:
                ...
                continue
            if b.get('If') == "level":
                a_cs += f"{b.get('Name')} - {b.get('Value', 0)}レベルになると達成できる\n"
            elif b.get('If') == "say":
                a_cs += f"{b.get('Name')} - {b.get('Value', 0)}回話すと達成できる\n"
            elif b.get('If') == "react":
                a_cs += f"{b.get('Name')} - {b.get('Value', 0)}回リアクションすると達成できる\n"
        await interaction.followup.send(embed=discord.Embed(title="実績リスト", description=a_cs, color=discord.Color.gold()))
        return
