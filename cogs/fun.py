...
class TextGroup(app_commands.Group):
    ...
    async def retranslate(self, interaction: discord.Interaction, テキスト: str):
        ...
        for lang in langs:
            ...
            word_ = await loop.run_in_executor(None, partial(GoogleTranslator, source="auto", target=lang))
    ...
    async def reencode(self, interaction: discord.Interaction, テキスト: str):
        ...
        await interaction.response.send_message(embed=discord.Embed(title="文字化け", description=encode(テキスト).decode("sjis", errors="ignore"), color=discord.Color.green()))
    ...
    async def dog(self, interaction: discord.Interaction):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get("https://dog.ceo/api/breeds/image/random") as dog_:
                await interaction.response.send_message(embed=discord.Embed(title="犬の画像", color=discord.Color.green()).set_image(url=f"{json.loads(await dog_.text())["message"]}"))
...
class FunCog(commands.Cog):
    ...
    async def janken(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(embed=discord.Embed(title="じゃんけん", description="""
・グーはチョキに勝ち、パーに負けます
・チョキはパーに勝ち、グーに負けます
・パーはグーに勝ち、チョキに負けます
